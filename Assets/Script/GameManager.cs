    ...
    void Start()
    {
        ...
        foreach (Tile t in Tiles)
        {
            ...
            foreach (Transform g in t.GetComponentsInChildren<Transform>())
            {
                ...
                if (g.name == "north") //TOP
                {
                    g.transform.Translate(new Vector3(-0.3f, 0, 0), Space.Self);
                }
                if (g.name == "west") // LEFT
                {
                    g.transform.Translate(new Vector3(0, 0, -0.3f), Space.Self);
                }
        ...
        if (!CurrPlayer.placedMeep && CurrPlayer.placedTile && CurrPlayer.meeples.Count > 0)
        {
            ...
            foreach (Tile.directions side in CurrPlayer.lastPlacedTile.Sides)
            {
                ...
                foreach (Grid.connectionsList conn in temp)
                {
                    ...
                    /*  if (conn.tile.placedMeepleChapel)
                      {
                          canPlaceMeepChapel = false;
                          break;
                      }
                      if (conn.tile.placedMeepleGrass)
                      {
                          canPlaceMeepField = false;
                          break;
                      }
                      if (conn.tile.placedMeepleRoad)
                      {
                          canPlaceMeepRoad = false;
                          break;
                      }*/
                }
            ...
            //Check if lastPlacedTile finished anything
            //Check each side for completion and count meeples in completed segment.
